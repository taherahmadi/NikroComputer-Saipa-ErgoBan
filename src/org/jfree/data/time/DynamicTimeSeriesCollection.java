...
public class DynamicTimeSeriesCollection extends AbstractIntervalXYDataset
	...
	public synchronized long setTimeBase(RegularTimePeriod start) {
		...
		long oldestL = this.pointsInTime[0]
				.getFirstMillisecond(this.workingCalendar);
		long nextL = this.pointsInTime[1]
				.getFirstMillisecond(this.workingCalendar);
	 ...
	 *            the series key.
	 * 
	 *            Use this as-is during setup only, or add the synchronized
	 *            keyword around the copy loop.
	 */
	@SuppressWarnings("rawtypes")
	public void addSeries(float[] values, int seriesNumber, Comparable seriesKey) {
		...
		if (this.valueHistory[seriesNumber] == null) {
			this.valueHistory[seriesNumber] = new ValueSequence(
					this.historyCount);
			this.seriesCount++;
		}
	...
	public void addValue(int seriesNumber, int index, float value) {
		...
		if (seriesNumber >= this.valueHistory.length) {
			throw new IllegalArgumentException(
					"TimeSeriesDataset.addValue(): series #" + seriesNumber
							+ "unspecified in c'tor");
		}
		if (this.valueHistory[seriesNumber] == null) {
			this.valueHistory[seriesNumber] = new ValueSequence(
					this.historyCount);
	 ...
	 * @param refresh
	 *            value of n in "refresh the display on every nth call" (ignored
	 *            if <= 0 )
	 */
