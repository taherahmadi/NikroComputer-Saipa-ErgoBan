...
public class ChartPanel extends JPanel implements ChartChangeListener,
	 ...
	 */
	public String doSaveAs() throws IOException {
	...
	public String doSaveAs(Object arg) throws IOException {
		JFileChooser fileChooser = new JFileChooser();
		fileChooser.setCurrentDirectory(this.defaultDirectoryForSaveAs);
		FileNameExtensionFilter filter = new FileNameExtensionFilter(
				"PDF files", "pdf");
	...
	private void saveAsSVG(File f) throws IOException {
		...
		if (file == null) {
			...
			if (option == JFileChooser.APPROVE_OPTION) {
				...
				if (file.exists()) {
					String fileExists = localizationResources
	 ...
	 *         <code>null</code> if there is a problem with the method
	...
	private Graphics2D createSVGGraphics2D(int w, int h) {
		try {
			Class svgGraphics2d = Class
					.forName("org.jfree.graphics2d.svg.SVGGraphics2D");
