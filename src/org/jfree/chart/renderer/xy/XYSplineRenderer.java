...
public class XYSplineRenderer extends XYLineAndShapeRenderer {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 358526725787665601L;
	...
	public static enum FillType {
		NONE, TO_ZERO, TO_LOWER_BOUND, TO_UPPER_BOUND
	}
	 ...
	 *            the number of points between data items.
	 * @param fillType
	 *            the type of fill beneath the curve (<code>null</code> not
	 *            permitted).
	 * 
	 * @since 1.0.17
	 */
	 ...
	 * 
	 * @param fillType
	 *            the fill type (<code>null</code> not permitted).
	 * 
	 * @see #getFillType()
	 * 
	 * @since 1.0.17
	...
	protected void drawPrimaryLineAsPath(XYItemRendererState state,
		...
		if (!Double.isNaN(transX1) && !Double.isNaN(transY1)) {
			Point2D p = plot.getOrientation() == PlotOrientation.HORIZONTAL ? new Point2D.Float(
					(float) transY1, (float) transX1) : new Point2D.Float(
					(float) transX1, (float) transY1);
			...
			if (s.points.size() > 1) {
				...
				if (this.fillType == FillType.TO_ZERO) {
					...
					float yz = (float) yAxis.valueToJava2D(0, dataArea,
							yAxisLocation);
					origin = plot.getOrientation() == PlotOrientation.HORIZONTAL ? new Point2D.Float(
							yz, xz) : new Point2D.Float(xz, yz);
					...
					origin = plot.getOrientation() == PlotOrientation.HORIZONTAL ? new Point2D.Float(
							ylb, xlb) : new Point2D.Float(xlb, ylb);
					...
					origin = plot.getOrientation() == PlotOrientation.HORIZONTAL ? new Point2D.Float(
							yub, xub) : new Point2D.Float(xub, yub);
				}
				...
				if (this.fillType != FillType.NONE) {
					if (plot.getOrientation() == PlotOrientation.HORIZONTAL) {
						s.fillArea.lineTo(origin.getX(),
								s.points.get(s.points.size() - 1).getY());
					} else {
						s.fillArea.lineTo(s.points.get(s.points.size() - 1)
								.getX(), origin.getY());
					}
					s.fillArea.closePath();
				}
