...
public class JFreeChart implements Drawable, TitleChangeListener,
	 ...
	 * @since 1.0.16
	 */
	public static final RenderingHints.Key KEY_SUPPRESS_SHADOW_GENERATION = new RenderingHints.Key(
			0) {
		@Override
	...
	public void setAntiAlias(boolean flag) {
		...
		if (!flag && RenderingHints.VALUE_ANTIALIAS_OFF.equals(val) || flag
	 ...
	 * @param drawHeight
	 *            the height for drawing the chart (will be scaled to fit
	 *            image).
	 * @param info
		...
		if (this.notify) {
			...
			for (int i = listeners.length - 2; i >= 0; i -= 2) {
				if (listeners[i] == ChartChangeListener.class) {
					((ChartChangeListener) listeners[i + 1])
							.chartChanged(event);
				}
			}
		}
...
class JFreeChartInfo extends ProjectInfo {
	...
	public JFreeChartInfo() {
		...
		ResourceBundle resources = ResourceBundleWrapper
				.getBundle(baseResourceClass);
	...
	public Image getLogo() {
		...
		if (logo == null) {
			URL imageURL = this.getClass().getClassLoader()
					.getResource("org/jfree/chart/gorilla.jpg");
			if (imageURL != null) {
